...
export class PlannedTasksTool implements AdminTool {
      ...
      parameters: {
        ...
        properties: {
          ...
          stepStatus: {
            ...
            description: 'Status for updating a step'
          },
  ...
  private async createNewTask(args: {
    ...
    if (!args.title || !args.description || !args.todo?.steps || !Array.isArray(args.todo.steps)) {
      return {
        success: false,
        data: null,
        summary: 'Missing required fields: title, description, todo.steps array required'
      };
    }
    ...
    return {
      success: true,
      data: { taskId, taskPath, todo, metadata, action: 'new_task' },
  ...
  private async loadTask(args: { taskId?: string }): Promise<ToolResult> {
    ...
    try {
      ...
      return {
        success: true,
        data: { taskId: args.taskId, taskPath, description, metadata, todo, artifacts, action: 'load_task' },
        summary: `Loaded task: ${metadata.title} (${todo.steps.length} steps, ${artifacts.length} artifacts)`,
        metadata: { action: 'load_task', taskId: args.taskId, stepCount: todo.steps.length, artifactCount: artifacts.length, status: todo.status, repaired: modified }
      };
    } catch {
      return { success: false, data: null, summary: `Failed to load task` };
    }
  }
  ...
  private async listTasks(): Promise<ToolResult> {
    ...
    const tasks = await Promise.all(tasksDir.directories.map(async dir => {
      ...
      try {
        ...
        return { taskId: dir, title: '(failed to read)', status: 'pending', updatedAt: 0 };
      }
    }));
